...
<body>
    <?php 
        ...
        // Eliminar usuario y validaciones actualizado para evitar errores
        if (isset($_POST['id'])) {
            $id_usuario = $_POST['id'];
            ...
            if (mysqli_query($cn, $query)) {
                ...
                if (strpos($error, 'foreign key') !== false || strpos($error, 'a foreign key constraint fails') !== false) {
                    echo '<script>alert("No se puede eliminar el usuario porque está asociado a una venta u otro registro."); window.location.href=window.location.href;</script>';
                    exit();
                } else {
                    echo '<script>alert("Error al eliminar usuario: ' . addslashes($error) . '"); window.location.href=window.location.href;</script>';
                    exit();
                }
            }
        }
        ...
        include_once '../includes/usuario_controller.php';
        /*include_once '../includes/editar_producto.php';*/
        include_once '../includes/eliminar_producto.php';
        //var_dump($_POST);
    ?>
    <div class="usuarios-page">
        <section class="head-usuario">
            <h1>Usuarios</h1>
        </section>
        <section class="filtros-usuario">
            <form method="POST" action="" class="form-filtros-usuario">
                <div class="buscadores1">
                    <div class="buscador-id">
                        ...
                        <button type="submit" class="btn-buscar-usuario">Buscar</button>
                    </div>
                    <div class="buscador-nombre">
                        <input type="text" name="buscar_nombre" placeholder="Buscar usuarios..." value="<?= htmlspecialchars($buscar_nombre) ?>">
                        <button type="submit" class="btn-buscar-usuario">Buscar</button>
                    </div>
                </div>
        ...
        <section class="mantenedor-usuario">
            <div class="btn-crear-usuario">
                <form action="" method="get">
                    <input type="submit" class="btn-crear" value="Crear Usuario" name="action">
                </form>
            </div>
        </section>
        <section class="table-usuarios">
            <table class="usuarios-table-main">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="100%">No se encontraron usuarios o hubo un error en la consulta.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </section>
        <?php include_once '../includes/paginacion_usuarios.php'; ?>
    </div>
    <?php mysqli_close($cn); ?>
</body>
</html>