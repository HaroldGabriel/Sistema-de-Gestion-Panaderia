<?php
    $result_total = mysqli_query($cn, "SELECT COUNT(*) AS total FROM cliente");
    $por_pagina = 5;
    ...
    */
    ...
    $result_total = mysqli_query($cn, "SELECT COUNT(*) AS total FROM cliente $where");
    $fila_total = mysqli_fetch_assoc($result_total);
    $total_registros = $fila_total['total'];
    $total_paginas = ceil($total_registros / $por_pagina);
    echo "<!-- WHERE generado: $where -->";
    $result_head = mysqli_query($cn, "SELECT id_cliente,nomb_cliente,ape_paterno,ape_materno,dni_cliente,email,telefono,dirección,id_ciudad,password_cliente FROM cliente $where LIMIT $inicio, $por_pagina");
    $result = mysqli_query($cn, "SELECT id_cliente,nomb_cliente,ape_paterno,ape_materno,dni_cliente,email,telefono,dirección,id_ciudad,password_cliente FROM cliente $where LIMIT $inicio, $por_pagina");
?>